...
def lambda_handler(event, context):
    ...
    try:
        ...
        write_manifest(event['ResourceProperties']['Manifest'], manifest_file)
        if event['RequestType'] == 'Create':
            run_command("kubectl create --save-config -f %s" % manifest_file)
        if event['RequestType'] == 'Update':
            run_command("kubectl apply -f %s" % manifest_file)
